...
def mysurveys(request):
    if request.user.is_authenticated:
        surveyposts = Survey.objects.filter(user_FK_id=request.user.id).annotate(participations=Count('question__answer__vote__user_FK_id', distinct=True), questioncount=Count('question__id', distinct=True)).order_by('-id')
        context = {}
        context['surveys'] = surveyposts
...
def joinsurvey(request, para):
    if request.user.is_authenticated:
        if Vote.objects.select_related('answer_FK__question_FK').filter(user_FK_id=request.user.id,answer_FK__question_FK__survey_FK_id=para).exists():
